    ...
    for (_, locations) in antenna_locations {
        for (cy, cx) in &locations {
            for (oy, ox) in &locations {
                if cy == oy && cx == ox {
                    continue;
                } else if antinode_grid[*cy][*cx] != '#' {
                    antinode_grid[*cy][*cx] = '#';
                    ans += 1;
                }
                let dy = *oy as i32 - *cy as i32;
                let dx = *ox as i32 - *cx as i32;
                let mut test_y = *cy as i32 - dy;
                let mut test_x = *cx as i32 - dx;

                while within_grid_bounds(&antinode_grid, test_y, test_x) {
